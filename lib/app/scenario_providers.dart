...
class ScenarioTrainingState {
  ...
  bool get canRevealMoreHints => hintStage < 2;
...
class ScenarioTrainingController extends Notifier<ScenarioTrainingState> {
  ...
  Future<void> toggleMic() async {
    ...
    if (state.isBlankFillMode && state.selectedBlankIndex == null) {
      final idx = _firstEmptyBlankIndex();
      if (idx == null) {
        state = state.copyWith(
    ...
    state = state.copyWith(
      ...
      clearSpokenText: !state.isBlankFillMode,
      clearError: true,
      soundLevel: 0,
      isVolumeLow: false,
      isTypingMode: false,
      ...
      await _stt.startListening(
        ...
        onResult: (text, {required bool isFinal}) {
          ...
          if (state.isBlankFillMode) {
            ...
            if (isFinal) {
              _stt.stopListening().then((_) => _submitSelectedBlank(line));
            }
          } else {
            state = state.copyWith(spokenText: text);
            if (isFinal) {
              _onSttFinal(text, line);
            }
          }
        },
      );
  ...
  /// 힌트 보기 — blank_frame 구조를 언더라인에 공개하고 빈칸 터치 모드로 전환.
  void revealNextHint() {
